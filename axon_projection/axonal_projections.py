...
import numpy as np
from neurom.core.morphology import iter_sections, Section
from collections import Counter
from voxcell import VoxelData, RegionMap
from choose_hierarchy_level import *
import os
import configparser
import sys

def create_ap_table(morph_dir, atlas_path, atlas_regions, atlas_hierarchy, hierarchy_level, output_path):
    """ Creates the axonal projections table for the morphologies found in 'morph_dir', at the given brain regions 'hierarchy_level'.
        ...
        Args:
            ...
            hierarchy_level (int): the desired hierarchy level of brain regions for the table (i.e. "granularity" of brain regions). 0 <=> root. Max is 11.
        ...
        Outputs:
            ...
            ap_check.csv: a file that says how many of OOB terminal points we have for each morpho, and is used to compare with manual annotation in check_atlas.py
            regions_names.csv: a file that basically lists all acronyms and names of brain regions used. This is just for information.
    """
    ...
    rows = []
    # list that will contain entries of the dataframe to compare with manual annotation. Also contains data about number of out of bound regions found for each morphology.
    rows_check = []
    ...
    for morph_file in list_morphs:
        ...
        # if morph doesn't have a soma, take the average of the first points of each sections of basal dendrites as source pos
        if source_pos is None or (isinstance(source_pos, list) and len(source_pos)==0):
            filter = lambda n : n.type == nm.BASAL_DENDRITE# or n.type == nm.APICAL_DENDRITE
            source_pos = np.mean([sec.points[0] for sec in iter_sections(morph, neurite_filter=filter)], axis = 0)[0:3]  # exclude radius if it is present
        ...
        try:
            ...
            if (repr(e).__contains__('Region ID not found') or
                    repr(e).__contains__('Out')):
                num_bad_morphs += 1
        ...
        # find the targetted region by each terminal
        filter = lambda n : n.type == nm.AXON
        # first find the terminal points
        terminal_points = [sec.points.tolist() for sec in iter_sections(morph, iterator_type = Section.ileaf, neurite_filter=filter)]
        ...
        terminals_regions = []
        # for some reason, it didn't work to give the term_pts_list to the lookup, it still considered it as a unhashable ndarray
        ...
        for term_pt in term_pts_list:
            ...
            try:
                term_pt_asc = region_map.get(brain_regions.lookup(term_pt), "acronym", with_ascendants=True)

                # add this terminal point's region to the region_names dict, with his ascendants
                if term_pt_asc[0] not in region_names.keys():
                    region_names[term_pt_asc[0]] = [region_map.get(brain_regions.lookup(term_pt), "acronym", with_ascendants=True), region_map.get(brain_regions.lookup(term_pt), "name", with_ascendants=True)]
                
                # get the acronym of the terminal pt's region at the desired brain region hierarchy level
                ...
                # and store it in the list of targetted regions of this morph
        ...
        rows_check.append({"source" : source_region, "OOB" : nb_oob_pts, "morph" : morph_file, "name" : morph_name_without_extension})

    if num_bad_morphs > 0:
        logging.info("Skipped %s morphologies that couldn't be loaded.", num_bad_morphs)
    if num_morphs_wo_axon > 0:
        logging.info("Skipped %s morphologies that did not have axon terminal points.", num_morphs_wo_axon)
    logging.info("Extracted projection pattern from %s axons.", len(list_morphs)-num_bad_morphs-num_morphs_wo_axon)
...
def main(config):
    ...
    create_ap_table(morph_dir, atlas_path, atlas_regions, atlas_hierarchy, hierarchy_level, output_path)
