...
def extract_acronyms_at_level(data, level):
    """ Extracts the list of all acronyms at hierarchy level 'level' of the brain regions in the json file 'data'.
...
def get_region_at_level(list_asc, level):
    """ Recursive function that gets the brain region at the given hierarchy 'level'. It is recursive because if 'list_asc' doesn't go as deep as 'level', we return the brain region at the closest hierarchy level.
    
        Args:
            list_asc (list<str>): a list of the ascendants of the brain regions of the brain region of interest (which is the first element of this list).
            level (int): the hierarchy level at which we want the region. (0 is root, max is 11)
        
        Returns:
            str: the brain region acronym at desired hierarchy level, or closest one (in direction of root).
    """
    ...
    acronyms_at_level = extract_acronyms_at_level('mba_hierarchy.json', level)
    # if one of the acronym in the ascendants list is within the acronyms at the given level, return this acronym
    for acr in list_asc:
